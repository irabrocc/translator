...
应用状态和操作入口集成在 Windows 任务栏通知区域。绿色 `F` 图标表示快速模式，
橙色 `T` 图标表示思考模式；切换模式时图标会立即更新。将鼠标悬停在图标上可以
查看当前模式、模型和语言方向，右键可以执行截图、数学解析或切换设置。若 Windows
将图标收进通知区域的折叠菜单，可在任务栏设置中将 Screenshot Translator 固定显示。

应用不再创建悬浮状态窗口，因此全屏游戏、视频和任务栏自动隐藏均遵循系统行为。
